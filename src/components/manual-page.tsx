  ...
  return (
    <section className="mb-12 border-b border-border pb-10">
      ...
      <p className="mt-4 max-w-2xl text-pretty text-base leading-relaxed text-muted">
        學員怎麼玩、資料哪裡來、模擬跟實盤差在哪。盤面像券商現股當沖；加權用證交所每 5 秒指數；撮合是教室的；實盤 API 還沒接。
      </p>
...
function Brief() {
  return (
    <Section id="brief" n="00" title="教室一頁說明">
      ...
      <blockquote className="mt-6 border-l-2 border-tape pl-4 text-sm leading-relaxed text-fg">
        一句話：路徑用官方指數、盤面像真的、下單是教室的。講師如果聽到「已經可以實盤下單」，那是講錯了。
      </blockquote>
    </Section>
  );
}
...
function Data() {
  return (
    <Section id="data" n="04" title="資料哪裡來">
      <p className="text-pretty leading-relaxed text-muted">
        跟講師討論時這張表最重要。教室用真實大盤當骨架，個股不是證交所逐筆。講成「跟券商看的一模一樣」，是誇大。
      </p>
      <div className="mt-5 overflow-x-auto rounded-lg border border-border">
        <table className="w-full min-w-xl text-left text-sm">
          <thead className="bg-surface-2 text-micro tracking-wide text-muted">
...
function Agenda() {
  return (
    <Section id="agenda" n="09" title="講師要對的五件事">
      <ol className="space-y-4 text-sm leading-relaxed">
        ...
        <Step n="3" title="要不要接哪一家券商？">
          下單畫面已預留。選哪一家、測試帳、正式帳、誰付 API 費，是講師跟券商的事。教室端只換 adapter。
        </Step>
        <Step n="4" title="學員資料放哪？">
          學員用 Email 登入，戰績寫回伺服器。老師用班級代碼 TEACHER 看儀表板。跨機戰績與排行已開。
        </Step>
...
function Colophon() {
  return (
    <footer className="mt-16 border-t border-border pt-6 text-micro leading-relaxed text-subtle">
      ...
      <p className="mt-1">本文件可列印、可下載 PDF，給講師當面過。內容以產品現況為準，實盤未接線前請勿對外宣稱可下真單。</p>
    </footer>
  );
}
